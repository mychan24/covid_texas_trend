...
if(max(df$Positive.Rate.Per.Day_7davg,na.rm=T)>30){
  ...
  g1 <-  df %>%
    ...
    ggrepel::geom_text_repel(data=df_texas[nrow(df_texas),], label="TX", hjust=1) + 
    ...
    annotate(geom = "text", x= end_date-35, y=ptr_y-1.8, label=most_recent_ptr_tx, color="grey50", size=4.5, fontface="bold", hjust=0) + 
    # plot settings
