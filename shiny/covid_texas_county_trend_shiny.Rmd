...
end_date <- as.Date(df$Date[nrow(df)])
...
if(max(df_texas$Positive.Rate.Per.Day_7davg,na.rm=T)>30){
  ...
  g4 <- df_texas %>%
    ...
    geom_hline(yintercept = 10, color="grey", linetype="dashed") +
    geom_hline(yintercept = 20, color="grey", linetype="dashed") +
    annotate(geom = "text", x = as.Date("2020-04-24"), y = 1, label = "Low", color="darkgreen", fontface="bold") + 
    annotate(geom = "text", x = as.Date("2020-04-25"), y = 6, label = "Medium", color="orange", fontface="bold") + 
    annotate(geom = "text", x = as.Date("2020-04-24"), y = 15, label = "High", color="#bb5516", fontface="bold") + 
    annotate(geom = "text", x = as.Date("2020-04-25"), y = 25, label = "Critical", color="red", fontface="bold") + 
    # --- background color for levels --- # 
    annotate(geom = "rect", xmin = as.Date("2020-04-22"), xmax =  as.Date(df$Date[nrow(df)]), ymin = -Inf, ymax = 3,
             fill = "darkgreen", colour = NA, alpha = 0.1) +  
    annotate(geom = "rect", xmin = as.Date("2020-04-22"), xmax =  as.Date(df$Date[nrow(df)]), ymin = 3, ymax = 10,
             fill = "orange", colour = NA, alpha = 0.1) +  
    annotate(geom = "rect", xmin = as.Date("2020-04-22"), xmax =  as.Date(df$Date[nrow(df)]), ymin = 10, ymax = 20,
             fill = "#bb5516", colour = NA, alpha = 0.1) +  
    annotate(geom = "rect", xmin = as.Date("2020-04-22"), xmax =  as.Date(df$Date[nrow(df)]), ymin = 20, ymax = Inf,
    ...
    scale_color_manual(values = c("Low" = "darkgreen", "Medium" = "orange", "High" = "#bb5516", "Critical" = "red")) +
    scale_x_date(name = 'Date', date_breaks = '2 days', date_labels = '%m-%d', expand = c(0,.5),
                 limits = c(as.Date("2020-04-22"), as.Date(df$Date[nrow(df)]))) +
    scale_y_continuous(labels = function(x) paste0(x, "%")) +
    ylab("Positive Rate Per Day(%)\n(New Case/New Test Per Day)") +
    ggtitle(label = "Est. Positive Rate Per Day Over Time (7-day average)") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
