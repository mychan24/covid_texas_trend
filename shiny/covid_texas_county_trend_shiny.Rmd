...
t <- cbind(t, t_p912[,2:ncol(t_p912)])
...
# most recent date:
recentdates <- data.frame(data=c("case","t"), recent_date=c(names(case)[ncol(case)],  names(t)[ncol(t)]))
...
for(cc in 1:length(counties)){
  ...
  df <- data.frame(Date=names(case)[(which(names(case)=="04-21")):which(names(case)==update_until)],
                   Total.Test.Performed = as.numeric(t[t$County==counties[cc],2:which(names(t)==update_until)]),
                   Total.Cases = as.numeric(case[which(case$`County Name`==counties[cc]),(which(names(case)=="04-21")):which(names(case)==update_until)]))
...
if(max(df$Positive.Rate.Per.Day_7davg,na.rm=T)>30){
  ...
  g1 <-  df %>%
    ...
    # Texas
    geom_line(data=df_texas, aes(x=Date, y=Positive.Rate.Per.Day_7davg, color=Positive.Rate.Level)) + 
    # geom_dl(data=df_texas, label="TX", method = list(dl.combine("last.points"))) + 
    # geom_text(data=df_texas[nrow(df_texas),], label="TX", hjust=0.5) + 
    ...
    # annotate(geom = "text", x = as.Date("2020-06-26"), color="firebrick",  
    #          y = 21, label = "TX Mask Mandate") +    
    ...
    # ------------------------------------ # 
    # text for ptr
    annotate(geom = "text", x= end_date-35, y=ptr_y, label=most_recent_ptr, color="grey20", size=4.5, fontface="bold", hjust=0) + 
    annotate(geom = "text", x= end_date-35, y=ptr_y-3, label=most_recent_ptr_tx, color="grey50", size=4.5, fontface="bold", hjust=0) + 
    # plot settings
