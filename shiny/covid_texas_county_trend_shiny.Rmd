...
<font size="-2"> __Disclaimer:__ These data were collected different (i) geographical locations, (ii) reported by different entities, and (iii) based only on 'individuals that got tested'. The trends should be interpreted or compared with great caution. These trends are only as accurate as the underlying "reported" cases/tests.</font>
...
most_recent_ptr <- sprintf("%s Positive Rate: %.f%%", format(as.Date(df$Date[nrow(df)]), "%B %d, %Y"), df$Positive.Rate.Per.Day_7davg[nrow(df)])
  ...
  g1 <- df %>%
    ...
    # text for ptr
    annotate(geom = "text", x= as.Date(df$Date[nrow(df)-8]), y=30, label=most_recent_ptr, color="grey20", size=4.5) + 
    # plot settings
