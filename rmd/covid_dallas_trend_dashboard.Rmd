...
### Dallas County, TX: Total Cases / New Confirmed Cases Over Time {.mobile}
```{r, fig.width=12, fig.height=7}
g3 <- df %>%
  # filter(`County Name`=="Dallas") %>% # only get dallas's data
    ggplot(aes(x=Date, y=Total.Cases)) +
...
### Texas State New COVID Tests and Positive Test-Rate Over Time {.mobile}
...
This report is generated using data from the [Texas Department of State Health Services](https://dshs.texas.gov/coronavirus/additionaldata/).

One of the most quoted COVID metric in the media has been “Rate of Positive Tests”. However, most websites or data dashboards have been largely reporting state-level positive test rates. Metropolitan areas with large populations can likely generate its own positive test rate data, which may differ from the state-level summary. 

Currently, this summary only includes Dallas County, TX. The next step would be to make this interactive and could show reports for other counties, if data is available. 

This report will automatically update daily if the data is available.