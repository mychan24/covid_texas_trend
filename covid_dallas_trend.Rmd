...
g1 <- case_long %>%
  ...
  geom_point() +
  geom_vline(xintercept = "04-30", color="firebrick", linetype="dashed", alpha=0.8) +
  annotate(geom = "text", x = "05-04", y = 0, label = "Re-open") +
  xlab("Date") +
  ylab("Total Cases") +
...
g2 <- d %>%
  ...
  geom_segment(data=d, mapping=aes(x="04-28", y=205, xend="04-30", yend=190),  # when Texas re-opened
               arrow=arrow(length = unit(0.2, "inches")), size=1, color="firebrick") +
  annotate(geom = "text", x = "04-28", y = 230, label = "Re-open") +
  xlab("Date") +
  ylab("New Cases") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
g6 <- df %>%
        ...
        geom_vline(xintercept = "04-30", color="firebrick", linetype="dashed", alpha=0.8) +
        annotate(geom = "text", x = "05-02", y = 0, label = "Re-open") +
        ylim(0,25) +
...
g9 <- df_texas %>%
        ...
        geom_bar(stat = "identity", fill="steelblue") +
        ylab("New Test Per Day\n(5-day average)") +
        geom_segment(data=d, mapping=aes(x="04-28", y=1300, xend="04-30", yend=1100),  # when Texas re-opened
                     arrow=arrow(length = unit(0.2, "inches")), size=1, color="firebrick") +
        annotate(geom = "text", x = "04-28", y = 1500, label = "Re-open") +
        theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
